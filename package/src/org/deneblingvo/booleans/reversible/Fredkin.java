...
public final class Fredkin extends TruthTable {
	...
	public void initDefs(Element defs, int elementWidth, int elementHeight) {
		if (!Image.containsDef(defs, getClass().getCanonicalName())) {
			...
			g.appendChild(Image.cross(defs.getOwnerDocument(), elementWidth / 2, elementHeight + (elementHeight / 2), elementHeight / 16));
			g.appendChild(Image.cross(defs.getOwnerDocument(), elementWidth / 2, (2 * elementHeight) + (elementHeight / 2), elementHeight / 16));
		}
